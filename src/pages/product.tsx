...
import RenderconstiantGroup from "../components/variants";
import {
  OrderEntity,
  ProductEntity,
  StoreEntity,
...
function Product({
  ...
  useEffect(() => {
    ...
    //     value: getPriceWithoutconstiantsDiscount(),
    //     currency: 'DZD'
    // });
    // on scrol
  }, []);
  ...
  function getPriceWithoutconstiantsDiscount(): number {
    return getProductPriceWithoutconstiantsDiscount(product!, item.constiants);
  }
  function getPriceAfterDiscount(): number {
    return getProductPriceAfterDiscount(product!, item.constiants);
  }

  function getQuantity(): number {
    return getProductQuantity(product!, item.constiants);
  }
  ...
  function getDiscount() {
    return (
      100 -
      getProductDiscountPercentage(product!, item.constiants) * 100
    ).toFixed(1);
  }
  ...
  async function sendOrder(status: "draft" | "pending" = "pending") {
    ...
    const localStorageKey = `order:${status}-${product.id}`;
    // allow send orders max 1/hour
    const olderOrder = localStorage.getItem(localStorageKey);
    if (olderOrder) {
      const orderData = JSON.parse(olderOrder) as OrderEntity;
      ...
      items: [
        {
          ...
          constiantPath: item.constiants.join("/"),
        },
      ],
      metadata: {
        metaPixel: {
    ...
    const response = await ff.orders.send(data);
      ...
      contents: [
        {
          id: product?.id,
          quantity: item.quantity,
          item_price: getPriceWithoutconstiantsDiscount(),
        },
      ],
      currency: "DZD",
      num_items: 1,
      ...
      >
        ...
        <div className="flex items-center justify-center">
          ...
          <span
            ...
            style={{ backgroundColor: "const(--on-p)" }}
          >
            x{item.quantity}
          </span>
        </div>
              ...
              <div className="absolute bottom-0 w-full flex justify-center p-2 items-end">
                {product?.media.map((media, index) => (
                  <a
                    ...
                    onClick={(e) => {
                      e.preventDefault();
                      const el = document.getElementById(`pimage-${index}`);
          ...
          <div className="w-full md:w-1/2 z-10">
            <div className="px-4 pt-5 product-gradient">
              ...
              <div className="flex items-center pb-3 ">
                ...
                {getPriceAfterDiscount() !==
                  getPriceWithoutconstiantsDiscount() && (
                  <span className="px-1  text-gray-400 line-through text-lg">
                    {getPriceWithoutconstiantsDiscount()} دج
                  </span>
                )}
            ...
            <div className="product-color">
              {product?.constiant && (
                <div className="gb p-4 rounded-xl">
                  ...
                  <div className="h-2"></div>
                  {/* constiant groups */}
                  <RenderconstiantGroup
                    constiantGroup={product!.constiant!}
                    path={item.constiants}
                    onPathChange={(path) => {
                      if (item.constiants.join() == path.join()) {
                        // delete last constiant
                        path.pop();
                      }
                      item.constiants = path;

                      return setItem({ ...item });
                    }}
                    onSelect={(constiant) => {
                      if (constiant?.type == constiantOptionType.image) {
                        const mediaIndex = product?.media.findIndex(
                          (media) => media == constiant!.value
                        );
                        console.log(constiant!.value);
                      ...
                      // ViewContent
                      ReactPixel.track("ViewContent", {
                        content_name: product?.name + " " + constiant?.name,
                        // content_category: 'cloth',
                        content_ids: [product?.id],
                        content_type: "product",
                        value: getPriceWithoutconstiantsDiscount(),
